...
export function Projects() {
  ...
  return (
    <div className="w-full flex flex-col gap-10 mt-5 px-20">
      ...
      <div>
        <ul className="flex flex-row justify-center gap-10 items-center sm:px-63 flex-wrap">
          {projectsRight.map((project, index) => {
            return (
              <li className="flex flex-row justify-between items-center gap-10 " key={index}>
                <img
                  src={project.img}
                  alt=""
                  className="w-[600px] h-[546px] rounded-md sm:max-w-[560px] sm:h-[300px]"
                />
